  ...
  public:
    ...
    **/
      ...
      settings_(settings)
    {
      pinMode(pin, OUTPUT);
      digitalWriteFast(pin, 1);
      DMASPI_PRINT(("ActiveLowChipSelect1 cs pin is %d\n", pin));
    }
...
//  {
//    txChannel_()->disable();
//    txChannel_()->destination((volatile uint8_t&)SPI1_);
//    txChannel_()->disableOnCompletion();
...
public:
  ...
  {
    txChannel_()->disable();
    txChannel_()->destination((volatile uint8_t&)SPI2_);
    txChannel_()->disableOnCompletion();
...
#error Unknown or unsupported chip
